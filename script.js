...
app.getRandomWord = () => {
  ...
  return app.randomWord = randomWordArray[Math.floor(Math.random() * randomWordArray.length)];
};

app.startTimer = () => {
  app.highScore = $score;

  app.halfMinute = setInterval(function() {
    $timer--;
    $('.timer').text(`Timer: ${$timer}`);
    if ($timer === 0){
      $('input').attr('disabled', true);
      clearInterval(app.halfMinute);
      ...
      if ($score > app.highScore) {
        app.highScore = $score;
        $('.highscore').text(`Highscore: ${app.highScore}`);
      }
    } 
  }, 1000);
}
...
app.submitWord = () => {
  app.userWord = ($('input').val()).toLowerCase();

  // prevents app.StartTimer() from running more than once
  if (parseInt($timer) === 30 && app.userWord === 'start') {
    app.startTimer();
  }

  if (app.userWord === app.randomWord) {
    $score++;
    randomWord = app.getRandomWord();

    $('.correct-word').text(app.userWord);
    $('input').attr('placeholder', app.randomWord);
